//TODO port to wgpu+winit
fn main() {}

// use euclid::{Box2D, Transform2D, UnknownUnit};
// use glium::{Surface, glutin};
// use lyon_path::math::{Angle, Point, Vector};
// use msdfgen::{Contour, PathCollector, compute_msdf, path::PathSegment, recolor_contours};
...
// fn get_glyph(font: &Face<'static>, chr: char) -> Vec<Contour> {
//     let id = font.glyph_index(chr).unwrap();

//     let mut builder = PathCollector::new();
//     font.outline_glyph(id, &mut builder).unwrap();

//     builder.finish()
// }

// fn bounds_for_contours(contours: &[Contour]) -> Box2D<f32, UnknownUnit> {
//     use euclid::{Rect, Size2D};
//     let mut elements = contours.iter().flat_map(|x| x.elements.iter());
//     let initial = {
//         let p = match elements.next() {
//             Some(x) => x.sample(0.0),
//             None => return Box2D::new(Point::new(0.0, 0.0), Point::new(0.0, 0.0)),
//         };
//         Box2D::new(p, Point::new(0.0, 0.0))
//     };
//     elements.fold(initial, |acc, x| match x.segment {
//         PathSegment::Line(s) => acc.union(&s.bounding_box()),
//         PathSegment::Quadratic(s) => acc.union(&s.bounding_box()),
//         PathSegment::Cubic(s) => acc.union(&s.bounding_box()),
//         PathSegment::Arc(s) => acc.union(&s.bounding_box()),
//     })
// }

// fn rescale_contours(mut contours: Vec<Contour>, bounds: Rect<f32>) -> Vec<Contour> {
//     let initial_bounds = bounds_for_contours(&contours);
//     let initial_scale = initial_bounds
//         .size()
//         .width
//         .max(initial_bounds.size().height);
//     let bounds_scale = bounds.size.width.max(bounds.size.height);
//     let transformation =
//         euclid::Transform2D::translation(-initial_bounds.min.x, -initial_bounds.min.y)
//             .then_scale(bounds_scale / initial_scale, bounds_scale / initial_scale)
//             .then_translate(bounds.origin.to_vector());
//     for contour in &mut contours {
//         for elem in &mut contour.elements {
//             elem.segment = match elem.segment {
//                 PathSegment::Line(s) => PathSegment::Line(s.transformed(&transformation)),
//                 PathSegment::Quadratic(s) => PathSegment::Quadratic(s.transformed(&transformation)),
//                 PathSegment::Cubic(s) => PathSegment::Cubic(s.transformed(&transformation)),
//                 PathSegment::Arc(s) => PathSegment::Arc(lyon_geom::Arc {
...
//         contours,
//         euclid::Rect::new(Point::new(0.2, 0.2), euclid::Size2D::new(0.6, 0.6)),
//     );
...
// mod test {
//     use super::*;
//     use lyon_geom::LinePathSegment;

//     fn line_segment(p0: Point, p1: Point) -> PathSegment {
//         PathSegment::Line(LinePathSegment { from: p0, to: p1 })
//     }
