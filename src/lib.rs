...
pub fn compute_msdf_for(
    ...
    sample_iter
        .map(|(px, py)| {
            ...
            [mmsd.r / 0.5f32, mmsd.g / 0.5, mmsd.b / 0.5]
        })
        .collect()
}
...
/// within [0, 1] x [0, 1]
pub fn compute_msdf(contours: &[Contour], dim: usize) -> Vec<[f32; 3]> {
    compute_msdf_for(
        contours,
        (0..dim).flat_map(|v| {
            let py = (v as f32 + 0.5) / dim as f32;
            (0..dim).map(move |v| ((v as f32 + 0.5) / dim as f32, py))
        }),
    )
}
